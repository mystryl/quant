#!/usr/bin/env python3
"""
增强版 SuperTrend 多年份多频率回测

测试 2023、2024、2025 年，15min 和 60min 频率
与标准版 SuperTrend 进行对比
"""
import pandas as pd
import numpy as np
...
def run_enhanced_backtest_for_params(df, strategy_name, results_list):
    """运行增强版回测"""
...
def main():
    ...
    for year in years:
        for freq in freqs:
            ...
            strategy_name = f"SuperTrend_SF14Re(50,20,n=3,ts=80)"
            run_enhanced_backtest_for_params(df, strategy_name, all_results)
    ...
    if len(all_results) > 0:
        ...
        results_df['freq'] = results_df['strategy_name'].str.extract(r'(\d+min)$', expand=False)
        results_df['year'] = results_df['strategy_name'].str.extract(r'\((\d+)', expand=False)
        ...
        output_file = Path("/mnt/d/quant/qlib_backtest/supertrend_enhanced_results.csv")
        results_df.to_csv(output_file, index=False, encoding='utf-8-sig')
        print(f"   结果已保存到: {output_file}")

    # 打印汇总表
    print(f"\n{'='*80}")
    print("增强版 SuperTrend 回测结果汇总")
    print(f"{'='*80}")
    ...
    for results in all_results:
        ...
        print(f"{results['strategy_name'][-4:]:<6} {data_length:>7}行 "
