"""
准备 MSB+OB 策略需要的 RB9999 数据。
从 Qlib 格式合并并重采样为 60 分钟级别（2023-2025年）。
"""
...
def prepare_strategy_format(df: pd.DataFrame) -> pd.DataFrame:
    """
    转换为策略回测需要的格式。
...
def main():
    ...
    # 输出目录
    output_dir = Path('/mnt/d/quant/projects/qlib_msb_ob/data')
    ...
    for year in years:
        ...
        # 转换为策略格式
        df_strategy = prepare_strategy_format(df_resampled)

        # 保存
        output_file = output_dir / f'RB9999_{year}_60min.csv'
