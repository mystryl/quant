...
df.loc[df['momentum'] > 0, 'position'] = 1    # 做多
df.loc[df['momentum'] < 0, 'position'] = -1   # 做空
```
...
df.loc[df['z_score'] < -2, 'position'] = 1   # 做多
df.loc[df['z_score'] > 2, 'position'] = -1   # 做空
```
...
df.loc[df['price_change'] > 0, 'position'] = 1    # 价格上涨，做多
df.loc[df['price_change'] < 0, 'position'] = -1   # 价格下跌，做空
```
...
df.loc[df['time_diff'] >= 60, 'position'] = 1
```
...
```python
# 与信号策略结合
signal_strategy = EnhancedIndexingStrategy()
executor = TWAPStrategy()

# 信号策略决定买卖
target_position = signal_strategy.generate_position(df)

# TWAP 分批执行
actual_orders = executor.generate_orders(target_position)
```
...
### Enhanced Indexing vs 布林带

| 指标 | Enhanced Indexing | 布林带 | 差异 |
|--------|-----------------|---------|------|
| 累计收益 | 92.18% | 92.18% | 0% |
| 年化收益 | 61.18% | 61.18% | 0% |
| 最大回撤 | 2.25% | 2.25% | 0% |
...
**结论：**
- Enhanced Indexing 与布林带策略**完全相同**
- 实现都是均值回归（Z-score = ±2）
- 差异在于交易次数（966 vs 9,668）
- **交易次数越少，成本越低，收益越高**

**为什么交易次数不同？**
```python
# Enhanced Indexing（我们的实现）
df.loc[df['z_score'] < -2, 'position'] = 1   # 只在突破时交易
df.loc[df['z_score'] > 2, 'position'] = -1

# 布林带（之前的实现）
df.loc[df['close'] < df['lower_band'], 'position'] = 1    # 每次突破都交易
df.loc[df['close'] > df['upper_band'], 'position'] = -1
```

**结论：** 应该减少交易频率，只在信号变化时交易。
...
1. **使用 Enhanced Indexing 策略**（均值回归）
   - ✅ 在震荡市场表现优异
   - ✅ 风险控制良好
   - ✅ 适合 1-5 分钟级别
